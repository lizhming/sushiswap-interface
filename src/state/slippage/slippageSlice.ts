...
export const GLOBAL_DEFAULT_SLIPPAGE = new Percent(50, 10_000) // .5%
...
export const slippageSlice = createSlice({
  ...
  reducers: {
    ...
    formatSlippageInput: (state) => {
      try {
        state.input = parseFloat(state.input).toFixed(2)
      } catch (e) {}
    },
  },
})
...
const selectSlippageInputError = (state: AppState): SlippageError | false => {
  try {
    ...
    return (!Number.isInteger(parsedInput) || parsedInput < 1 || parsedInput > 5000) && state.slippage.input !== '.'
...
export const selectSlippage = (state: AppState): Percent =>
  selectSlippageInputError(state) ? GLOBAL_DEFAULT_SLIPPAGE : inputToPercent(state)
