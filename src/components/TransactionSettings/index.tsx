...
const TransactionSettings: FC<TransactionSettingsProps> = ({ placeholderSlippage, trident = false }) => {
  ...
  const slippageIsDefault = slippagePercent.equalTo(GLOBAL_DEFAULT_SLIPPAGE)
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            <div className="flex items-center justify-between gap-1">
              ...
              <input
                ...
                value={slippageInput.length > 0 ? slippageInput : slippageIsDefault ? '' : slippagePercent.toFixed(2)}
                onChange={(e) => dispatch(setSlippageInput(e.target.value))}
                onBlur={() =>
                  slippageError
                    ? dispatch(setSlippageInput(GLOBAL_DEFAULT_SLIPPAGE.toFixed(2)))
          ...
          <div>
            <Button
              ...
              variant="outlined"
              onClick={() => dispatch(setSlippageInput(GLOBAL_DEFAULT_SLIPPAGE.toFixed(2)))}
            >
              {i18n._(t`Auto`)}
            </Button>
          </div>
